/* === METRA Phase 9.9A.3 – Inline Date-Stamp Ladder ===
   Scalable single-user to corporate architecture.
   Focus: per-field audit visibility while preserving simplicity.
   Module: Change Control Popup (inline timestamps for Notes, Email, Link, and Status)
   Verified baseline: 2025-10-30-changecontrol-popup-expandable-ladder-v9.9A.2
   Next target: 2025-10-31-changecontrol-popup-datestamp-v9.9A.3
--------------------------------------------------------------- */

import React, { useState, useEffect } from "react";

export default function TaskPopupChangeTest() {
  const STORAGE_KEY = "metra_change_steps_v9_9A3";

  const defaultSteps = [
    { id: 1, title: "Impact Assessment", status: "Not Started", notes: "", email: "", link: "",
      timestamps: {}, open: false },
    { id: 2, title: "Board Review", status: "Not Started", notes: "", email: "", link: "",
      timestamps: {}, open: false },
    { id: 3, title: "Implementation Verification", status: "Not Started", notes: "", email: "", link: "",
      timestamps: {}, open: false },
  ];
      ...
      prev.map((s) =>
        s.id === id
          ? {
              ...s,
              [field]: value,
              timestamps: { ...s.timestamps, [field]: `Updated ${currentTime()}` },
            }
          : s
      )
    );
      ...
      prev.map((s) =>
        s.id === id
          ? {
              ...
              timestamps: {
                ...s.timestamps,
                status: `Status changed to ${status} – ${currentTime()}`,
              },
            }
          : s
      )
  ...
  const addStep = () => {
    const newStep = {
      ...
      email: "",
      link: "",
      timestamps: { created: `Created ${currentTime()}` },
      open: true,
    };
  ...
  return (
    <div className="popup-container">
          ...
          <div key={s.id} className="step-card">
            <div className="step-row" onClick={() => toggleOpen(s.id)}>
              ...
              <input
                ...
                onBlur={(e) => handleChange(s.id, "title", e.target.value)}
              />
              <span className="status-label">{s.status}</span>
              <span className="saved-flag">
                {savedFlags[`${s.id}_title`] && "✔ Saved"}
              </span>
            </div>
            ...
            >
              <div className="expand-inner">
                ...
                {s.timestamps.status && (
                  <div className="timestamp-line">🕓 {s.timestamps.status}</div>
                )}
                ...
                <div className="field-block">
                  ...
                  <textarea
                    rows="4"
                    value={s.notes}
                    onChange={(e) => handleChange(s.id, "notes", e.target.value)}
                  />
                  <span className="saved-flag">
                ...
                {s.timestamps.notes && (
                  <div className="timestamp-line">🕓 {s.timestamps.notes}</div>
                )}
                ...
                {s.timestamps.email && (
                  <div className="timestamp-line">🕓 {s.timestamps.email}</div>
                )}
                ...
                <div className="field-block">
                  ...
                  <input
                    ...
                    placeholder="https://"
                    value={s.link}
                    onChange={(e) => handleChange(s.id, "link", e.target.value)}
                  />
                  <span className="saved-flag">
                ...
                {s.timestamps.link && (
                  <div className="timestamp-line">🕓 {s.timestamps.link}</div>
                )}
              </div>
            </div>
          </div>
      ...
      <style jsx>{`
        ...
        .step-row:hover { background: #dce7fb; }
        ...
        .collapse-cue { cursor: pointer; color: #0a2b5c; font-size: 1rem; }
        .expand-inner { padding: 0.6rem 0.8rem; background: #fff; }
        .field-block { display: flex; align-items: center; margin-top: 0.4rem; }
        .field-block label { width: 110px; font-size: 0.9rem; font-weight: 500; }
        ...
        .saved-flag {
          ...
          font-size: 0.85rem; color: #0a8d2a;
          margin-left: 0.3rem;
        }
        .status-buttons {
        ...
        button {
          background: #0a2b5c; color: white; border: none;
